...
\begin{proof}
    閉包の性質から$\symcal{F} \subset \closureop{\symcal{F}}$なので，
    \cref{thm:Hilbert-subset-inclusion-orthogonal-compliment}より$(\closureop{\symcal{F}})^\perp \subset \symcal{F}^\perp$がいえる．
    逆の包含を示す．
    $\symbf{x} \in \symcal{F}^\perp$とすると，
    任意の$\symbf{v} \in \symcal{F}$に対し，$\iparen{\symbf{v}, \symbf{x}} = 0$である．
    ここから任意の$\symbf{v}_* \in \closureop{\symcal{F}}$に対して$\iparen{\symbf{v}_*, \symbf{x}} = 0$を示せば，
    $\symbf{x} \in (\closureop{\symcal{F}})^\perp$がいえる．
    これは内積の連続性から示される．
\end{proof}
...
\begin{proposition}
    ...
    $(\symcal{F}^\perp)^\perp = \closureop{\symcal{F}}$である．
\end{proposition}

\begin{proof}
    $\symbf{v} \in (\symcal{F}^\perp)^\perp$をとる．
    $\closureop{\symcal{F}}$は閉部分ベクトル空間であるから，
    \refdfn[正射影定理]{thm:Hilbert-projection-theorem}より，
    \[
        \symbf{z} \in \symbf{x} + \symbf{y},
        \quad 
        \text{where } 
        \symbf{x} \in \closureop{\symcal{F}}, \  
        \symbf{y} \in (\closureop{\symcal{F}})^\perp = \symcal{F}^\perp
    \]
...
\begin{proposition}
    ...
    $\symcal{F}^\perp = \{ \symbf{0} \}$となることである．
\end{proposition}

\begin{proof}
    \textsf{必要}　$\symcal{F}$が稠密であるとする．
    任意の$\symbf{x} \in \symcal{H}$に対し，
    $\symcal{F}$の点列$\sequ{\symbf{x}_n}$が存在して，
    $\symbf{x}_n \xrightarrow{n \to \infty} \symbf{x}$となる．
    そこで，$\symbf{v} \in \symcal{F}^\perp$を任意にとると，
    直交補空間の定義から，すべての$\symbf{x}_n \in \symcal{F}$に対して$\iparen{\symbf{v}, \symbf{x}} = 0$である．
    ここで，\refdfn[内積の連続性]{thm:inner-product-continuity}より，
    \begin{equation*}
        \iparen{\symbf{v}, \symbf{x}}
        = \iparen{\symbf{v}, \  \lim_{n \to \infty} \symbf{x}_n}
        = \lim_{n \to \infty} \iparen{\symbf{v}, \symbf{x}_n}
        = 0
    \end{equation*}
    $\symbf{x} \in \symcal{H}$は任意であったので，$\symbf{v} = \symbf{0}$．

    \textsf{十分}　$\symcal{F}^\perp$は閉部分ベクトル空間である\refdfn{thm:Hilbert-subspace-orthogonal-compliment}から，
    \refdfn[正射影定理]{thm:lemma-of-projection-theorem}より，
    任意の$\symbf{z} \in \symcal{F}^\perp$は
    \[  
        \symbf{z} = \symbf{x} + \symbf{y}, 
        \quad \text{where }
        \symbf{x} \in \symcal{F}^\perp, \  \symbf{y} \in (\symcal{F}^\perp)^\perp
    \]
\end{proof}
...
\begin{proof}
    ...
    一方，\refthm[内積の連続性]{thm:inner-product-continuity}および$\sequ{\symbf{u}_n}$が正規直交系であることから，
    \begin{equation*}
        \norm{\symbf{v}} 
        = \lim_{N \to \infty} \norm{\symbf{v}_N}
