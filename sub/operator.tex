...
\begin{definition}[エルミート共役]
    ...
    この$\adjoint{\hat{T}}$を，$\hat{T}$の\word{エルミート共役}(Hermitian adjoint)\index{えるみいと@エルミート!きようやく@共役}という．
\end{definition}

エルミート共役$\adjoint{\hat{T}}$が存在する条件について考えてみよう．
$\hat{T}$が\refdfn-[有界]{dfn:bounded-operator}な演算子であるとすれば，
$\symbf{y}$を固定した時の関数
\[  
    f_{\symbf{y}} (\symbf{x}) \coloneq \iparen{ \symbf{y}, \, \hat{T} \symbf{x} }_{\symcal{K}} 
\]
は$\symcal{K}$から$\symbb{K}$への線形汎関数であるから，
\refdfn[リースの表現定理]{thm:Riesz-representation-theorem}より，
