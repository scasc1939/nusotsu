...
\begin{proposition}
    2乗可積分関数空間$L^2 (𝛺)$は，以下で定義される内積$(\bigdot, \bigdot)$に対して\refdfn[内積空間]{dfn:inner-product}になる．
    \begin{align}
...
$L^2$上の\refdfn[同値関係]{dfn:equivalence-relation}$\sim$を
\begin{equation*}
    f \sim g  \iff  f(\symbf{x}) = g(\symbf{x}) \quad (\text{a.e. } \symbf{x} \in \Omega)
\end{equation*}
と定義する．$\sim$が同値関係になるのは明らかである．
\refdfn[商集合]{dfn:quotient-set}$L^2/{\sim}$上の和とスカラー倍を，
代表元を用いて
\begin{align}
