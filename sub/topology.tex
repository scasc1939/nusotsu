...
\begin{definition}[開集合と閉集合]
    ...
    $(X, d)$を距離空間とする．
    \begin{enumerate}[ref={\cref{dfn:open-set-and-closed-set}.\arabic*}]
        \item \label{dfn:open-set}
            $A \subset X$の内部が$A$自身と一致するとき，
            $A$は\word{開集合}[][かいしゆうこう](open set)であるとは，
        \item \label{dfn:closed-set}
            $A \subset X$の$A$の閉包$\cl{A}$が$A$と一致するとき，
            $A$は\word{閉集合}[][へいしゆうこう](closed set)である
    \end{enumerate}
\end{definition}

開集合\refdfn{dfn:open-set}の補集合は閉集合である。
逆に、閉集合の補集合は開集合である。
