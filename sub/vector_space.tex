...
\begin{definition}[ベクトル空間の生成系]
    ...
    ベクトルの組$𝒗_1, 𝒗_2, \dotsc \in V$が$W$を\word{生成する}(generate)あるいは\word{張る}(span)とは，
...
\begin{definition}[一次独立と一次従属]
    ...
    $V$を$𝕂$上のベクトル空間とする．
    ベクトルの組$𝒗_1, 𝒗_2, \dotsc \in V$が\word{一次独立}[いち|じ|どく|りつ](linearly independent)あるいは\word{線形独立}であるとは，
    ...
    \end{equation*}
    であることをいう．
    ベクトルの組$𝒗_1, 𝒗_2, \dotsc \in V$が一次独立でないとき，\word{一次従属}[いち|じ|じゅう|ぞく](linearly dependent)あるいは\word{線形従属}という．
\end{definition}
...
\begin{definition}[基底]
    \label{dfn:basis}
    $V$を$𝕂$上のベクトル空間とする．
    ベクトルの組$ \{ 𝒗_1, 𝒗_2, \dotsc \} \subset V$は，以下を満たすとき，\word{基底}[き|てい](basis)という．
    \begin{enumerate}
