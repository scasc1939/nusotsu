...
\subsection{3次元ユークリッド空間$\symbb{R}^3$の性質}

この節では，ユークリッド空間$\symbb{R}^3$における数ベクトルの性質を確かめる．
...
\begin{definition}[ベクトル空間]
    ...
    \begin{enumerate}
        ...
        \item \label{vector:sum-zero} ある$\symbf{0} \in V$がただひとつ存在し，任意の$\symbf{v} \in V$に対して，$\symbf{v} \vecplus \symbf{0} = \symbf{0} \vecplus \symbf{v} = \symbf{v}$である．
        \item \label{vector:sum-opposite} 任意の$\symbf{v} \in V$に対して，ある$-\symbf{v}$がただひとつ存在して，$\symbf{v} \vecplus (-\symbf{v}) = (-\symbf{v}) \vecplus \symbf{v} = \symbf{0}$である．
...
$c \scaprod \symbf{v} \eqcolon c \symbf{v}$とかく．
...
したがって，ベクトル$\symbf{v}_1, \dots, \symbf{v}_n$がベクトル空間$V$に属するなら，
それらの線形結合$c_1 \symbf{v}_1 \vecplus \dotsb \vecplus c_n \symbf{v}_n$も$V$に属する．
...
\begin{example}
    複素数全体の集合$\symbb{C}$は，
    $\symbb{C}$上のベクトル空間と考えると$\dim_{\symbb{C}} \symbb{C} = 1$（基底$1$），
    $\symbb{R}$上のベクトル空間と考えると$\dim_{\symbb{R}} \symbb{C} = 2$（基底$1, \iu$）である．
\end{example}
