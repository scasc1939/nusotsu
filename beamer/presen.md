...
また，1つのベクトルの複素数倍（スカラー倍）をすることもできます．やはりベクトル $\boldsymbol{x}$ を複素数倍したものもベクトルになっています．
...
同じように，同型を使うと，量子力学の基本となる線形写像（線形演算子）は，行列として書くことができることがわかります．
...
というわけで，これでめでたく，ヒルベルト空間として縦ベクトルと行列を使えばよいことになりました．
...
そこで，これからは，行列を使った量子力学について，考えてみます．
...
まず始めに，量子力学において，状態はヒルベルト空間における単位ベクトルとしてあらわされる，というのは，皆さんもご存じでしょう．
...
ここで線形代数学の知識から，エルミート行列の固有ベクトルは，考えているヒルベルト空間の正規直交基底になるということがわかります．
...
### 波束の収縮
公理の最後として，波束の収縮という，量子力学の非常に不思議なところを定義します．
...
この測定によって，実験者は $A$ の固有状態のうちのひとつ， $\ket{\varphi_a}$ に対応する値を観測するはずです（固有ベクトルに縮退がない場合）．

面白いことに，その瞬間，オブザーバブル $A$ の固有状態 $\ket{\varphi_a}$ の線形結合で書けていた系の状態が，観測した固有状態 $\ket{\varphi_a}$ に変化（収縮）してしまいます．
...
量子多体系（といっても2体系）の取り扱い方について考えます．


### スピン系

話を簡単にするために，これからはスピン系について考えることにします．
...
というわけで，それぞれの状態を2次元縦ベクトル $(1, 0)$ ， $(0, 1)$ に対応させることにしましょう．

こんなできるのは，2次元ヒルベルト空間が $\mathbb{C}^2$ と同型だったからですね．

(*NEXT*)

もちろん，アップとダウンを重ね合わせた状態も，このように縦ベクトルで書けますね．
...
1粒子の扱い方は，量子力学で最初から扱われますし，多数（$N_A$＝アボガドロ数程度）の粒子の扱い方は，統計力学で取り上げられます．

その真ん中，2粒子の扱い方というのはあまりまじめに取り上げられていないような気がします．

(*NEXT*)

こうした複合粒子を扱うのには，テンソル積とよばれる演算を使います．

テンソル積の定義はここに書いたとおりで，特に補足するところもありません．

1粒子の状態は2次元ベクトルですが，2粒子の状態は4次元になるんですね．

3粒子，4粒子についても，テンソル積を帰納的に使えば定義できます．


#### テンソル積の空間

テンソル積の基底というものを考えることもできます．

上の式は，1粒子のヒルベルト空間 $\mathbb{C}^2$ の基底を，それぞれ組み合わせた4つですね．

上の4つをそれぞれ計算すれば，下の4つになります．こう書いてしまえばただの $\mathbb{C}^4$ の基底ですね．

基底ということは， $\mathbb{C}^2$ と $\mathbb{C}^2$ テンソル積の空間が $\mathbb{C}^4$ ということです．
...
ヒルベルト空間 $\mathbb{C}^2$ と $\mathbb{C}^2$ のテンソル積をとると， $\mathbb{C}^4$ のベクトルになります．

ところが， $\mathbb{C}^4$ が，2つのベクトルのテンソル積として書けるとは限りません．

ここには $(0, 1, 1, 0)$ という4次元ベクトルを書きましたが，これは2つの2次元ベクトルのテンソル積として書けません．

これが $(a, b) \otimes (c, d)$ と書けたと仮定しましょう．まず $ac = 0$ なので， $a = 0$ または $c = 0$ です．ところが， $a = 0$ なら $ad = 1$ に矛盾しますし， $c = 0$ なら $bc = 0$ に矛盾します．
...
$(0, 1, 1, 0)$ を，スピン系の状態で書けば，このようになります（ $\ket{\uparrow} = (1, 0)$, $\ket{\downarrow} = (0, 1)$ を使う）．もちろん，先ほどと同じように，この状態は2つのスピン状態のテンソル積として書くことができません．
...
このような状態に対して，スピンの向きが上向きか下向きかを測定することを考えます．

まず，粒子は2つありますが，当然，どちらのスピンを測定するかを選ぶことができます．とりあえず粒子 A を測定することにしましょう．

測定によって， A のスピンの向きが上か下かが確定します．たとえば上向きだったとしましょう．

すると，波束の収縮によって，複合状態が A のスピンが上向きという状態に収束します．つまり， $\ket{\uparrow}_{\mathrm{A}}$ を含む項だけが残り，あとは消えます．

この状況では， $\ket{\uparrow}_{\mathrm{A}} \otimes \ket{\downarrow}_{\mathrm{B}}$ という項しか残りません．

つまり，粒子 B のスピンは下向き，ということです．

A のスピンが上向きだという情報を得た時点で， B のスピンは下向きだと決まってしまうのです．

最初の A のスピンが下向きだったとしても，やはり B のスピンは確定してしまいます（上向き）．

このように，エンタングル状態とは，2粒子が“もつれている”状態であると言えます．
